...
const Settings = () => {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${window.ENV?.VITE_API_URL || import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/auth/change-password`, {
        method: 'POST',
        headers: {
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${window.ENV?.VITE_API_URL || import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/auth/update-profile`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
